...
```text
YouTubeSimpleDownloader_Setup_v0.9.5-inno-self-signed.exe
```
...
Avoid using older releases to process untrusted local video files. The responsible-use restrictions below still apply to downloaded content.
...
```powershell
Get-FileHash .\YouTubeSimpleDownloader_Setup_v0.9.5-inno-self-signed.exe -Algorithm SHA256
```
...
```text
release\YouTubeSimpleDownloader_Setup_v0.9.5-inno-self-signed.exe
```
